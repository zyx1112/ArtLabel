...
class EncodeProcessDecode(snt.AbstractModule):
  ...
  def _build(self, input_op, num_processing_steps): 
    ...
    ave_pool1D=tf.layers.AveragePooling1D(pool_size=16,strides=1,padding='SAME')
    temp1=[]
    ...
    for i in range(num_processing_steps):
       ...
       #latent1 = self._core(core_input)
        temp1=[]
        for idx,v in enumerate(core_input):
            if(idx==0):
                new_v=tf.reshape(v,[-1,32])
                temp1.append(new_v)
            elif(idx==1):
                new_v=tf.reshape(v,[-1,32])
                temp1.append(new_v)
                ...
                new_v=tf.reshape(v,[-1])
                temp1.append(new_v)
        core_input=graph_nets.graphs.GraphsTuple(*temp1)
        latent1 = self._core(core_input)  
